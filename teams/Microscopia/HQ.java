...
public class HQ {
	...
	public static void run() throws GameActionException {
		...
		if (Pastr.enemyPastrCount() > 0) {
			...
			if (Movement.locToInt(enemyPastrLocation) != rc.readBroadcast(Broadcast.HQ_PATH_LOCATION_CHANNEL[PathFind.ENEMY_PASTR_PATH_NUM])) {
	...
	public static void doYourThing() throws GameActionException {
		...
		if (currentRoundNum > lastRoundCalculatedCount + 50) {
			rc.broadcast(Broadcast.IS_TOWER_BUILT_CHANNEL, NoiseTower.isTowerBuiltFull() ? 1 : 0);
			rc.broadcast(Broadcast.IS_PASTR_BUILT_CHANNEL, Pastr.isPastrBuiltFull() ? 1 : 0);
			rc.broadcast(Broadcast.ENEMY_PASTR_COUNT, Pastr.enemyPastrCountFull());
